...
describe("listScheduledMessagesSchema", () => {
  ...
  it("validates channelId filter", () => {
    const res = listScheduledMessagesSchema.safeParse({ channelId: "123456789012345678901234" });
    expect(res.success).toBe(true);
  });

  it("validates channel filter", () => {
    const res = listScheduledMessagesSchema.safeParse({ channel: "general" });
    expect(res.success).toBe(true);
  });

  it("fails if both channel and channelId are provided", () => {
    const res = listScheduledMessagesSchema.safeParse({ channel: "general", channelId: "123" });
    expect(res.success).toBe(false);
  });

  it("validates userId filter", () => {
    const res = listScheduledMessagesSchema.safeParse({ userId: "668e30546a5ea56c5d83f46b" });
    expect(res.success).toBe(true);
  });
