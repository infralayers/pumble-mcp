...
describe("replyMessageSchema", () => {
  it("accepts valid input and defaults asBot to false", () => {
    const result = replyMessageSchema.parse({ messageId: "msg1", channelIdentifier: "c1", text: "hi" });
...
describe("replyMessage", () => {
  ...
  it("calls POST /sendReply with the parsed input", async () => {
    ...
    const input = replyMessageSchema.parse({ messageId: "msg1", channelIdentifier: "c1", text: "hello" });
    ...
    expect(JSON.parse(options.body)).toEqual({ messageId: "msg1", channelId: "c1", text: "hello", asBot: false });
  });
});
