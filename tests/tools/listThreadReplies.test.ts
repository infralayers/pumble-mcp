...
describe("listThreadRepliesSchema", () => {
  it("accepts valid input", () => {
    expect(listThreadRepliesSchema.safeParse({ messageId: "msg1", channelIdentifier: "c1" }).success).toBe(true);
  });
});
...
describe("listThreadReplies", () => {
  ...
  it("calls GET /fetchThreadReplies with query params", async () => {
    ...
    const input = listThreadRepliesSchema.parse({ messageId: "msg1", channelIdentifier: "c1", limit: 50 });
    ...
    expect(urlString).toContain("channelId=c1");
